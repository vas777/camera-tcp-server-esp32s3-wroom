...
pub async fn face_detection(
    ...
    loop {
        let image_to_tcp = image_to_tcp.clone();
        let d = detector.clone();
        match rec.recv().await {
            Ok(frame) => {
                task::spawn_blocking(move || {
                    let mut image =
                        image::DynamicImage::from(image::load_from_memory(&frame).unwrap());

                    let faces = d.lock().unwrap().detect(&image).unwrap();
                    ...
                    imageproc::drawing::draw_hollow_rect_mut(
                        &mut image,
                        Rect::at(
                            absolute_coordinates_of_face[0].0,
                            absolute_coordinates_of_face[0].1,
                        )
                        .of_size(
                            absolute_coordinates_of_face[0].2,
                            absolute_coordinates_of_face[0].3,
                        ),
